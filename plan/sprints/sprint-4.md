...
**Status:** In progress — planning approved; contract and operation slice next
...
|---|---:|---|---|
| CPS-401 VM create contract and validation | 8 | In progress | ownership checks, explicit network, safe optional fields, no user-data leakage |
| CPS-402 VM create operation workflow | 8 | In progress | 202 operation, reference-only command, idempotent terminal instance persistence |
| CPS-403 VM detail and power/delete workflows | 8 | In progress | state/capability checks, normalized result, tombstone and stable conflicts |
| CPS-404 Root-volume lifecycle persistence | 5 | Ready | boot source and delete-on-termination policy persist without blind volume delete |
| OPS-401 Create VM handler | 13 | In progress | IMAGE and VOLUME_FROM_IMAGE mapping, marker/replay safety, bounded user data |
| OPS-402 Instance detail operation | 5 | In progress | normalized instance plus related port/volume snapshots |
| OPS-403 Start and stop handlers | 8 | In progress | preconditions, waiter, replay-safe result |
| OPS-404 Reboot handler | 5 | In progress | reboot type mapping, waiter and normalized errors |
| OPS-405 Delete handler and root-volume semantics | 8 | In progress | Nova delete-on-termination semantics and related refresh |
...
- 2026-07-23: CPS operation completion now persists normalized instance results transactionally; OPS start/stop/reboot paths wait for target provider state and delete emits an idempotent DELETED result. Related port/volume snapshots, root-volume verification, and real VM acceptance remain open.
