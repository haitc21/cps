...
class VolumeAttachmentOperation(StrEnum):
    ATTACH = "attach"
    DETACH = "detach"
...
class VolumeAttachmentOperationRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = Field(default="1.0", pattern=r"^1\.0$")
    operation_id: UUID
    resource_type: str = Field(default="volume-attachment", pattern=r"^volume-attachment$")
