"""Provider-neutral, no-bytes Glance image lifecycle command contract."""
...
IMAGE_IMPORT_ALLOWED_HOSTS = frozenset(
    ...
    for host in os.getenv("CPS_IMAGE_IMPORT_ALLOWED_HOSTS", "images.example.test").split(",")
    if host.strip()
)
...
class ImageOperationRequest(BaseModel):
    """A strictly bounded image command; image content is never represented."""
    ...
    def validate_lifecycle(self) -> ImageOperationRequest:
        ...
        if self.operation in {ImageOperation.CREATE, ImageOperation.IMPORT_URL}:
            if not self.name or not self.disk_format:
